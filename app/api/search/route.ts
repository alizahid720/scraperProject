import { ContactDetail, LeadInput, saveLead } from '../../../db/leads';
...
function scheduledValidation<T>(request:()=>Promise<T>){const run=validationQueue.then(async()=>{await new Promise(resolve=>setTimeout(resolve,200));return request()});validationQueue=run.then(()=>undefined,()=>undefined);return run}
...
async function validatePhone(phone:string,keepTrustedOnError=false){
 ...
 try{
  ...
  const response=await scheduledValidation(()=>fetch(url,{headers:{accept:'application/json'},signal:AbortSignal.timeout(8000),cache:'no-store'}));
...
async function validatePhones(googlePhones:string[],websitePhones:string[]){
 ...
 const candidates=uniquePhones([...googlePhones,...websitePhones]).slice(0,4);
 ...
 return {
  ...
  verifiedKeys:new Set(checked.filter(result=>result.verified).map(result=>result.phone.replace(/\D/g,'').slice(-10)))
 };
}
...
type WebsiteContact={value:string;type:'phone'|'email'|'whatsapp';sourceUrl:string;scope:'Branch'|'Shared'};
async function enrichWebsite(website:string|undefined,context:EnrichmentContext){if(!website)return {emails:[],phones:[],whatsapps:[],details:[] as WebsiteContact[],matched:false};try{const base=safeWebsite(website);if(!base)return {emails:[],phones:[],whatsapps:[],details:[] as WebsiteContact[],matched:false};const home=await fetchHtml(base.toString()),pages=[{html:home,url:base}];const links=contactLinks(home,base);const linked=await Promise.all(links.map(async link=>({html:await fetchHtml(link),url:new URL(link)})));pages.push(...linked);return pages.reduce((all,page)=>{const scoped=relevantPage(page.html,page.url,context);if(!scoped)return all;const branch=contacts(scoped),pageContacts=contacts(page.html),branchPhones=new Set(branch.phones.map(value=>value.replace(/\D/g,'').slice(-10))),branchEmails=new Set(branch.emails),branchWhatsapps=new Set(branch.whatsapps.map(value=>value.replace(/\D/g,'').slice(-10))),details:WebsiteContact[]=[...pageContacts.phones.map(value=>({value,type:'phone' as const,sourceUrl:page.url.toString(),scope:branchPhones.has(value.replace(/\D/g,'').slice(-10))?'Branch' as const:'Shared' as const})),...pageContacts.emails.map(value=>({value,type:'email' as const,sourceUrl:page.url.toString(),scope:branchEmails.has(value)?'Branch' as const:'Shared' as const})),...pageContacts.whatsapps.map(value=>({value,type:'whatsapp' as const,sourceUrl:page.url.toString(),scope:branchWhatsapps.has(value.replace(/\D/g,'').slice(-10))?'Branch' as const:'Shared' as const}))];return {emails:unique([...all.emails,...pageContacts.emails]),phones:uniquePhones([...all.phones,...pageContacts.phones]),whatsapps:uniquePhones([...all.whatsapps,...pageContacts.whatsapps]),details:[...all.details,...details],matched:true}},{emails:[] as string[],phones:[] as string[],whatsapps:[] as string[],details:[] as WebsiteContact[],matched:false})}catch{return {emails:[],phones:[],whatsapps:[],details:[] as WebsiteContact[],matched:false}}}
async function enrichWebsites(websites:string[],context:EnrichmentContext){const results=await Promise.all(websites.map(website=>enrichWebsite(website,context)));return results.reduce((all,result)=>({emails:unique([...all.emails,...result.emails]),phones:uniquePhones([...all.phones,...result.phones]),whatsapps:uniquePhones([...all.whatsapps,...result.whatsapps]),details:[...all.details,...result.details],matched:all.matched||result.matched}),{emails:[] as string[],phones:[] as string[],whatsapps:[] as string[],details:[] as WebsiteContact[],matched:false})}
...
function contactEvidence(mapsUrl:string,listedPhones:string[],websiteDetails:WebsiteContact[],phones:string[],whatsapps:string[],emails:string[],verifiedPhoneKeys:Set<string>,verifiedEmails:Set<string>):ContactDetail[]{const listedKeys=new Set(listedPhones.map(value=>value.replace(/\D/g,'').slice(-10))),findWebsite=(type:WebsiteContact['type'],value:string)=>websiteDetails.find(detail=>detail.type===type&&(type==='email'?detail.value.toLowerCase()===value.toLowerCase():detail.value.replace(/\D/g,'').slice(-10)===value.replace(/\D/g,'').slice(-10)));const details:ContactDetail[]=[];for(const value of phones){const key=value.replace(/\D/g,'').slice(-10),website=findWebsite('phone',value),listed=listedKeys.has(key);details.push({value,type:'phone',source:listed?'Google Maps':'Official website',sourceUrl:listed?mapsUrl:website?.sourceUrl||'',scope:listed||website?.scope==='Branch'?'Branch':'Shared',validation:verifiedPhoneKeys.has(key)?'Verified':listed||website?'Source-matched':'Unverified'})}for(const value of emails){const website=findWebsite('email',value);details.push({value,type:'email',source:'Official website',sourceUrl:website?.sourceUrl||'',scope:website?.scope||'Shared',validation:verifiedEmails.has(value.toLowerCase())?'Verified':'Source-matched'})}for(const value of whatsapps){const website=findWebsite('whatsapp',value);details.push({value,type:'whatsapp',source:'Official website',sourceUrl:website?.sourceUrl||'',scope:website?.scope||'Shared',validation:'Source-matched'})}return details.filter((detail,index)=>details.findIndex(other=>other.type===detail.type&&other.value===detail.value)===index)}
...
async function outscraper(category:string,city:string,state:string,country:string,requested:number|null):Promise<LeadInput[]>{
 ...
 for(let i=0;i<places.length;i+=4){
  ...
  leads.push(...await Promise.all(batch.map(async place=>{
   ...
   const [validated,emailChecks]=await Promise.all([validatePhones(listedPhones,discoveredPhones),Promise.all(discoveredEmails.slice(0,4).map(email=>validateEmail(email)))]);
   ...
   const emails=unique(emailChecks.map(result=>result.email)).slice(0,4),emailVerified=emailChecks.some(result=>result.verified),verifiedEmails=new Set(emailChecks.filter(result=>result.verified).map(result=>result.email.toLowerCase()));
...
async function google(category:string,city:string,state:string,country:string,requested:number|null):Promise<LeadInput[]>{
 ...
 for(let i=0;i<places.length;i+=4){const batch=places.slice(i,i+4);leads.push(...await Promise.all(batch.map(async p=>{const businessName=p.displayName?.text||category,fullAddress=p.formattedAddress||`${city}, ${state}`,extra=await enrichWebsite(p.websiteUri,{businessName,fullAddress,city,state}),googlePhones=uniquePhones([p.internationalPhoneNumber||'',p.nationalPhoneNumber||'']).slice(0,1),[validated,emailChecks]=await Promise.all([validatePhones(googlePhones,extra.phones),Promise.all(extra.emails.slice(0,4).map(email=>validateEmail(email)))]),phones=validated.phones,phoneKeys=new Set(phones.map(phone=>phone.replace(/\D/g,'').slice(-10))),whatsapps=extra.whatsapps.filter(phone=>phoneKeys.has(phone.replace(/\D/g,'').slice(-10))).slice(0,4),emails=unique(emailChecks.map(result=>result.email)).slice(0,4),emailVerified=emailChecks.some(result=>result.verified),verifiedEmails=new Set(emailChecks.filter(result=>result.verified).map(result=>result.email.toLowerCase())),query=encodeURIComponent(`${businessName}, ${fullAddress}`),mapsUrl=`https://www.google.com/maps/search/?api=1&query=${query}&query_place_id=${encodeURIComponent(p.id)}`,hasVerifiedContact=validated.verified||emailVerified,contactDetails=contactEvidence(mapsUrl,googlePhones,extra.details,phones,whatsapps,emails,validated.verifiedKeys,verifiedEmails);return {businessName,placeId:p.id,mapsUrl,websites:p.websiteUri?[safeWebsite(p.websiteUri)?.toString()||p.websiteUri]:[],phones,whatsapps,emails,contactDetails,rating:p.rating,reviewCount:p.userRatingCount||0,fullAddress,country,state,city,category,status:hasVerifiedContact?'Validated':extra.matched&&(emails.length+phones.length>googlePhones.length)?'Enriched':'Collected',confidence:validated.verified&&emailVerified&&extra.matched?'High':hasVerifiedContact||extra.matched?'Medium':'Low'}})))}return leads;
}
...
function removeCrossBusinessContactLeaks(leads:LeadInput[]){
 ...
 for(const [key,owners] of phoneOwners){const uniqueOwners=[...new Set(owners)];if(uniqueOwners.length<2||uniqueOwners.every(index=>sameOrganization(leads[uniqueOwners[0]],leads[index])))continue;for(const index of uniqueOwners){const phones=leads[index].phones||[];if(phones[0]?.replace(/\D/g,'').slice(-10)!==key){leads[index].phones=phones.filter(phone=>phone.replace(/\D/g,'').slice(-10)!==key);leads[index].contactDetails=(leads[index].contactDetails||[]).filter(detail=>detail.type!=='phone'||detail.value.replace(/\D/g,'').slice(-10)!==key)}}}
 for(const [email,owners] of emailOwners){const uniqueOwners=[...new Set(owners)];if(uniqueOwners.length<2||uniqueOwners.every(index=>sameOrganization(leads[uniqueOwners[0]],leads[index])))continue;for(const index of uniqueOwners){leads[index].emails=(leads[index].emails||[]).filter(value=>value.toLowerCase()!==email);leads[index].contactDetails=(leads[index].contactDetails||[]).filter(detail=>detail.type!=='email'||detail.value.toLowerCase()!==email)}}
 return leads;
}
