...
async function validatePhones(googlePhones:string[],websitePhones:string[]){
 ...
 const checked=await Promise.all(candidates.map(phone=>validatePhone(phone,true)));
 return {
...
async function enrichWebsites(websites:string[]){const results=await Promise.all(websites.map(enrichWebsite));return results.reduce((all,result)=>({emails:unique([...all.emails,...result.emails]),phones:uniquePhones([...all.phones,...result.phones]),whatsapps:uniquePhones([...all.whatsapps,...result.whatsapps])}),{emails:[] as string[],phones:[] as string[],whatsapps:[] as string[]})}
...
async function google(category:string,city:string,state:string,country:string,requested:number|null):Promise<LeadInput[]>{
 ...
 for(let i=0;i<places.length;i+=4){const batch=places.slice(i,i+4);leads.push(...await Promise.all(batch.map(async p=>{const extra=await enrichWebsite(p.websiteUri),googlePhones=uniquePhones([p.internationalPhoneNumber||'',p.nationalPhoneNumber||'']).slice(0,1),[validated,emailChecks]=await Promise.all([validatePhones(googlePhones,extra.phones),Promise.all(extra.emails.slice(0,8).map(email=>validateEmail(email)))]),phones=validated.phones,phoneKeys=new Set(phones.map(phone=>phone.replace(/\D/g,'').slice(-10))),whatsapps=extra.whatsapps.filter(phone=>phoneKeys.has(phone.replace(/\D/g,'').slice(-10))).slice(0,4),emails=unique(emailChecks.map(result=>result.email)).slice(0,4),emailVerified=emailChecks.some(result=>result.verified),businessName=p.displayName?.text||category,fullAddress=p.formattedAddress||`${city}, ${state}`,query=encodeURIComponent(`${businessName}, ${fullAddress}`);return {businessName,placeId:p.id,mapsUrl:`https://www.google.com/maps/search/?api=1&query=${query}&query_place_id=${encodeURIComponent(p.id)}`,websites:p.websiteUri?[p.websiteUri]:[],phones,whatsapps,emails,rating:p.rating,reviewCount:p.userRatingCount||0,fullAddress,country,state,city,category,status:validated.verified||emailVerified?'Validated':whatsapps.length?'Enriched':'Collected',confidence:validated.verified&&emailVerified?'High':validated.verified||emailVerified||p.websiteUri?'Medium':'Low'}})))}return leads;
}
