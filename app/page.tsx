  ...
  <div className="shell" id="top">
   ...
   <section className="content">
    ...
    <section id="leads" className="leads">
     <div className="leads-head"><div><h2>Collected leads</h2><p>All discovered public contacts are preserved across repeat searches.</p></div><div className="filters"><input placeholder="Search leads" value={q} onChange={e=>setQ(e.target.value)}/><select value={status} onChange={e=>setStatus(e.target.value)}><option value="all">All statuses</option><option>Enriched</option><option>Collected</option></select><input type="date" value={from} onChange={e=>setFrom(e.target.value)}/><input type="date" value={to} onChange={e=>setTo(e.target.value)}/><button className="clear-button" type="button" disabled={!summary.total||deleting==='all'} onClick={()=>deleteData()}>{deleting==='all'?'Deleting…':'Clear all'}</button></div></div>
