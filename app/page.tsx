...
type ContactDetail={value:string;type:'phone'|'email'|'whatsapp';source:'Google Maps'|'Official website';sourceUrl:string;scope:'Branch'|'Shared';validation:'Verified'|'Source-matched'|'Unverified'};
type Lead={id:number;businessName:string;mapsUrl:string|null;websites:string[];phones:string[];whatsapps:string[];emails:string[];contactDetails:ContactDetail[];rating:number|null;reviewCount:number;fullAddress:string;country:string;state:string;city:string;category:string;status:string;confidence:string;createdAt:string};
...
const evidence=(lead:Lead,type:ContactDetail['type'],value:string)=>lead.contactDetails?.find(detail=>detail.type===type&&detail.value===value);
...
export default function Home(){
 ...
 useEffect(()=>{if(!countryCode)return;fetch(`/api/locations?type=states&country=${countryCode}`).then(r=>r.json()).then((d:{items:StateOption[]})=>{setStates(d.items||[]);if(!(d.items||[]).some(s=>s.isoCode===stateCode)){setStateCode('');setCity('')}}).finally(()=>setLocationsLoading(false))},[countryCode,stateCode]);
 useEffect(()=>{if(!countryCode||!stateCode)return;fetch(`/api/locations?type=cities&country=${countryCode}&state=${stateCode}`).then(r=>r.json()).then((d:{items:string[]})=>{setCities(d.items||[]);if(!(d.items||[]).includes(city))setCity('')}).finally(()=>setLocationsLoading(false))},[countryCode,stateCode,city]);
   ...
   <section className="content">
    ...
    <form id="search" onSubmit={search} className="search-card">
     ...
     <div className="fields">
      <label><span>Country</span><select aria-label="Country" value={countryCode} onChange={e=>{setLocationsLoading(true);setCountryCode(e.target.value);setStateCode('');setCity('')}} required><option value="">Select country</option>{countries.map(c=><option key={c.isoCode} value={c.isoCode}>{c.name}</option>)}</select><input type="hidden" name="country" value={countries.find(c=>c.isoCode===countryCode)?.name||''}/></label>
      <label><span>State / Province</span><select aria-label="State or province" value={stateCode} onChange={e=>{setLocationsLoading(true);setStateCode(e.target.value);setCity('')}} disabled={!countryCode||locationsLoading||!states.length} required><option value="">{locationsLoading?'Loading states…':states.length?'Select state':'No states available'}</option>{states.map(s=><option key={s.isoCode} value={s.isoCode}>{s.name}</option>)}</select><input type="hidden" name="state" value={states.find(s=>s.isoCode===stateCode)?.name||''}/></label>
    ...
    <section id="leads" className="leads">
     ...
     <div className="table"><table><thead><tr><th>Business</th><th>Location</th><th>All public contacts</th><th>Rating</th><th>Confidence</th><th>Collected</th><th>Action</th></tr></thead><tbody>
      {loading?<tr><td colSpan={7} className="empty">Loading workspace…</td></tr>:!leads.length?<tr><td colSpan={7} className="empty"><strong>No leads yet</strong><small>Run your first search above to collect live business data.</small></td></tr>:leads.map(l=><tr key={l.id}>
       ...
       <td className="contacts">{l.phones.map(p=>{const d=evidence(l,'phone',p);return <span key={p}>☎ {p}{d&&<small>{d.source} · {d.scope} · {d.validation}</small>}</span>})}{l.whatsapps.map(p=>{const d=evidence(l,'whatsapp',p);return <span key={'wa'+p}>◉ WhatsApp {p}{d&&<small>{d.scope} · {d.validation}</small>}</span>})}{l.emails.map(e=>{const d=evidence(l,'email',e);return <span key={e}>✉ {e}{d&&<small>{d.scope} · {d.validation}</small>}</span>})}{l.websites[0]&&<a href={l.websites[0]} target="_blank">Official website ↗</a>}{!l.phones.length&&!l.emails.length&&<small>No public contacts found</small>}</td>
